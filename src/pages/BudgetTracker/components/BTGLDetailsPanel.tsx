          ...
          btItems.reduce((map, item) => {
            ...
            return {
              ...
              requestedByMe: UtilsService.uniqByObjectKey(
                [
                  ...(map.requestedByMe || []),
                  ...(item.creator_id === user?.synergyId ? [item] : [])
                ],
                "id"
              ),
      ...
      <div className={"summary-table-wrapper"}>
        ...
        <Table className={"summary-table"}>
            ...
            {Object.values(btCategoryMap).map(btCategory => {
              return (
                <tr key={btCategory.id}>
                  ...
                  <td>{getCategoryItems(btItemMap.approved, btCategory, 'approved')}</td>
                  <td>{getCategoryItems(btItemMap.declined, btCategory, 'declined')}</td>
                  <td>{getCategoryItems(btItemMap.requested, btCategory, 'requested')}</td>
                  <td>{getCategoryItems(btItemMap.pending, btCategory, 'pending')}</td>
                  <td>
                    {getCategoryItems(btItemMap.requestedByMe, btCategory, 'requestedByMe')}
                  </td>
                  <td>
                    {getCategoryItems(btItemMap.requestedNotByMe, btCategory, 'requestedNotByMe')}
                  </td>
          ...
          <tfoot>
            <tr>
              <td>Total</td>
              <td>{getCategoryItems(btItemMap.approved, undefined, 'approved')}</td>
              <td>{getCategoryItems(btItemMap.declined, undefined, 'declined')}</td>
              <td>{getCategoryItems(btItemMap.requested, undefined, 'requested')}</td>
              <td>{getCategoryItems(btItemMap.pending, undefined, 'pending')}</td>
              <td>{getCategoryItems(btItemMap.requestedByMe, undefined, 'requestedByMe')}</td>
              <td>{getCategoryItems(btItemMap.requestedNotByMe, undefined, 'requestedNotByMe')}</td>
        ...
        <div className={"details-table"}>
          <h5>
            ...
            `${showingFilter.categoryGuid || ""}`.trim() in btCategoryMap
              ? <> in <u>{btCategoryMap[`${showingFilter.categoryGuid || ""}`.trim()].name}</u></>
              : ""}
          </h5>
          <BTItemsTable
            items={items}

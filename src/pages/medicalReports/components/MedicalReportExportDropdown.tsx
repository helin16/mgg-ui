...
const MedicalReportExportDropdown = ({students, conditionsMap}: iMedicalReportExportDropdown) => {
  ...
  const getBgColorObj = (colName: string, color: string) => {
    if (!colName.trim().toUpperCase().startsWith('D')) {
      return {};
    }
  ...
  const getColStyle = (ref: string, style: any, existingStyle?: any, severityColor?: string) => {
    return {
      ...(existingStyle || {}),
      ...style,
  ...
  const getRowStyle = (rowNo: number, style?: any, existingStyleMap?: any, severityColor?: string, cols = ['A', 'B', 'C', 'D', 'E']) => {
    const localStyle = {...(existingStyleMap || {})};
    cols.forEach(col => {
      const celRef = `${col}${rowNo}`;
      localStyle[celRef] = getColStyle(celRef, style || {}, localStyle[celRef] || {}, severityColor);
    })
    return localStyle;
  }
  ...
  const getRows = (startRowNo = 1, cols = ['A', 'B', 'C', 'D', 'E']) => {
    ...
    students.forEach((student, sIndex) => {
      ...
      if (conditionsMap[student.StudentID].length > 1) {
        mergeCells.push({s: {r: rowNo - 1, c: 0}, e:{r: rowNo + conditionsMap[student.StudentID].length - 2, c: 0}});
        mergeCells.push({s: {r: rowNo - 1, c: 1}, e:{r: rowNo + conditionsMap[student.StudentID].length - 2, c: 1}});
      }

      conditionsMap[student.StudentID].forEach((condition, index) => {
        const conditionArr = [condition.ConditionTypeDescription, condition.ConditionSeverityDescription, condition.ConditionDetails];
        if (index === 0) {
          cellStyleMap = getRowStyle(rowNo, borderStyleObj, cellStyleMap, condition.ConditionSeverityDisplayColour);
          rows.push([...studentArr, ...conditionArr])
        } else {
          cellStyleMap = getRowStyle(rowNo, undefined, cellStyleMap, condition.ConditionSeverityDisplayColour);
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(wb, `Student_Medical_Export_${moment().format('YYYY_MM_DD_HH_mm_ss')}.xlsx`);
  }

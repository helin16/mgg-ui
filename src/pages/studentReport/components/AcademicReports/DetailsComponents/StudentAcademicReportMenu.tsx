...
import {
  iStudentAcademicReportResultMap,
  StudentAcademicReportDetailsProps
} from "../StudentAcademicReportDetails";
...
const StudentAcademicReportMenu = ({
  ...
  useEffect(() => {
    const sortedCourses = _.flatMapDeep(
      Object.values(studentReportResultMap)
    ).sort((reprt1, reprt2) => {
    ...
    const setCourseMap = (
      ...
      setFn(
        courseList.filter(filterFn).reduce((map, studentReportResult) => {
          return {
            ...map,
    ...
    setCourseMap(
      sortedCourses,
      setHomeGroupCourseMap,
      course => course.isHomeGroup === true
    );
    setCourseMap(
      sortedCourses,
      setAcademicCourseMap,
      course =>
        course.isHomeGroup === false &&
        course.FileType === STUDENT_REPORT_RESULT_FILE_TYPE_ACADEMIC
    );
    setCourseMap(
      sortedCourses,
      setOtherCourseMap,
  ...
  const getItemClassName = (classCode: string) => {
    return selectedCourseCode === classCode
      ? "course-item active"
      : "course-item";
  };
    ...
    return (
      <li>
        <LinkBtn
          ...
          onClick={() =>
            onSelectedCourse(STUDENT_REPORT_SUBJECT_NAME_COMPARATIVE_ANALYSIS)
          }
        >
          {STUDENT_REPORT_SUBJECT_NAME_COMPARATIVE_ANALYSIS}
        </LinkBtn>
  ...
  const getListOfCourseNames = (
    courseMap: { [key: string]: iStudentReportResult },
    contactNames: boolean = false
  ) => {
    ...
    const getCourseName = (courseCode: string) => {
      ...
      if (contactNames === true) {
        return `${courseMap[courseCode].AssessHeading || ""} ${
          courseMap[courseCode].ClassDescription
        }`;
      }
      return `${courseMap[courseCode].AssessHeading || ""}`;
    };
    return courseCodes.map(courseCode => {
      ...
      return (
        <li key={courseCode}>
          {/*// @ts-ignore*/}
          <LinkBtn
            className={getItemClassName(courseCode)}
            onClick={() => onSelectedCourse(courseCode)}
          >
    ...
    if (user?.isStaff === true) {
      return (
        <Button
          variant={"danger"}
          onClick={() =>
            onClearSelectedStudent ? onClearSelectedStudent() : null
          }
        >
      ...
      return (
        <div>
          <div className="d-grid gap-2" style={{ marginTop: "1rem" }}>
            ...
            <Button
              variant={"danger"}
              onClick={() => (onClearReportYear ? onClearReportYear() : null)}
            >
              <Icon.ListUl /> All {student.StudentGiven1}'s Reports
            </Button>
            <div className="d-grid gap-2">{getBackToHomePage()}</div>
          </div>
        </div>
      );
    }
    ...
    return (
      <div>
        ...
        <div className="d-grid gap-2">
          <StudentReportDownloadBtn
            student={student}
            studentReportYear={studentReportYear}
          />
          <Button variant={"danger"} onClick={() => setShowingEmailPopup(true)}>
            <Icon.Envelope /> Email PDF Report
          </Button>
          <Button
            variant={"danger"}
            onClick={() => (onClearReportYear ? onClearReportYear() : null)}
  ...
  return (
    <Wrapper>
      ...
      <PanelTitle className={"ellipsis"}>
        {studentReportYear.FileYear} Semester{" "}
        {studentReportYear.FileSemester / 2} Report
      </PanelTitle>
      {getContent()}
    </Wrapper>
  );
